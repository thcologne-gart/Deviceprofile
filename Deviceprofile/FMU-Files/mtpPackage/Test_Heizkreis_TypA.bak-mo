...
//-----block heating circuit-----
  mtpPackage.Heizkreis_TypA heizkreis_TypA(
    ...
    redeclare AixLib.Systems.HydraulicModules.BaseClasses.PumpInterface_SpeedControlledNrpm PumpInterface(
      pump(
        redeclare package Medium = Medium,redeclare AixLib.Fluid.Movers.Data.Pumps.Wilo.Stratos25slash1to6 per)), 
    m_flow_nominal = 1, 
    ...
    T_amb = 293.15)
    annotation(
    Placement(visible = true, transformation(origin = {-1.77636e-15, -50}, extent = {{-50, -50}, {50, 50}}, rotation = 0)));
  //-----blocks pump bus-----
...
equation
  connect(boundaryOut.ports[1], heizkreis_TypA.port_b2) annotation(
    Line(points = {{-80, -80}, {-50, -80}}, color = {0, 127, 255}));
  connect(boundaryIn.ports[1], heizkreis_TypA.port_a1) annotation(
  ...
  connect(hydraulicBus, heizkreis_TypA.hydraulicBus) annotation(
  ...
  connect(heizkreis_TypA.port_b1, heizkreis_TypA.port_a2) annotation(
