...
model Kaeltekreis_TypB_withBoundary "Modell für Kältekreis Typ B-Beimischschaltung"
  ...
  AixLib.Fluid.FixedResistances.GenericPipe pipe5(redeclare package Medium = Medium, energyDynamics = Modelica.Fluid.Types.Dynamics.FixedInitial, length = 10, m_flow_nominal = 0.3, nNodes = 3, parameterIso = AixLib.DataBase.Pipes.Insulation.Iso50pc(), parameterPipe = AixLib.DataBase.Pipes.Copper.Copper_28x1(), pipeModel = "SimplePipe", withConvection = true, withInsulation = true) annotation(
    Placement(visible = true, transformation(origin = {-132, 0}, extent = {{10, -10}, {-10, 10}}, rotation = -90)));
  //-----pump-----
  ...
  Modelica.Blocks.Sources.Constant y(k = 1) annotation(
    Placement(visible = true, transformation(origin = {30, 80}, extent = {{-100, 40}, {-80, 60}}, rotation = 0)));
  //-----valves-----
  ...
  AixLib.Fluid.FixedResistances.PressureDrop hydraulicValve(redeclare package Medium = Medium, m_flow_nominal = 0.2, from_dp = true, dp_nominal = 10) "Fixed resistance" annotation(
  ...
  Modelica.Blocks.Sources.Ramp ramp(duration = 1, height = 1, offset = 0) annotation(
    Placement(visible = true, transformation(origin = {-140, -190}, extent = {{-40, 50}, {-20, 70}}, rotation = 0)));
...
equation
  ...
  connect(y.y, pump.y) annotation(
    Line(points = {{-49, 130}, {-31, 130}, {-31, 92}}, color = {0, 0, 127}));
  ...
  connect(prescribedTemperature.port, pipe2.heatPort) annotation(
    Line(points = {{120, 0}, {30, 0}, {30, 70}}, color = {191, 0, 0}));
  ...
  connect(pipe5.heatPort, prescribedTemperature.port) annotation(
    Line(points = {{-122, 0}, {120, 0}}, color = {191, 0, 0}));
  connect(ramp.y, valve.y) annotation(
    Line(points = {{-158, -130}, {-130, -130}, {-130, -92}}, color = {0, 0, 127}));
  connect(valve.port_2, pipe4.port_b) annotation(
    Line(points = {{-120, -80}, {-80, -80}}, color = {0, 127, 255}));
  connect(valve.port_1, port_b2) annotation(
    Line(points = {{-140, -80}, {-200, -80}}, color = {0, 127, 255}));
  connect(valve.port_3, pipe5.port_a) annotation(
    Line(points = {{-130, -70}, {-132, -70}, {-132, -10}}, color = {0, 127, 255}));
  connect(pipe2.port_b, senVolFlo.port_a) annotation(
    Line(points = {{40, 80}, {58, 80}}, color = {0, 127, 255}));
  connect(senVolFlo.port_b, senT_WMZ1.port_a) annotation(
    Line(points = {{78, 80}, {100, 80}}, color = {0, 127, 255}));
  connect(hydraulicValve.port_a, pipe3.port_b) annotation(
    Line(points = {{60, -80}, {100, -80}}, color = {0, 127, 255}));
  connect(hydraulicValve.port_b, senT_WMZ2.port_a) annotation(
    Line(points = {{40, -80}, {10, -80}}, color = {0, 127, 255}));
  connect(junc45.ports[1], port_a1) annotation(
    Line(points = {{-130, 80}, {-198, 80}}, color = {0, 127, 255}));
  connect(junc45.ports[2], pipe1.port_a) annotation(
    Line(points = {{-130, 80}, {-80, 80}}, color = {0, 127, 255}));
  connect(junc45.ports[3], pipe5.port_b) annotation(
    Line(points = {{-130, 80}, {-132, 80}, {-132, 10}}, color = {0, 127, 255}));
  connect(PT1_WMZ1.u, senT_WMZ1.T) annotation(
    Line(points = {{90, 118}, {90, 110}, {110, 110}, {110, 92}}, color = {0, 0, 127}));
  connect(PT1_b1.u, senT_b1.T) annotation(
    Line(points = {{150, 118}, {150, 91}}, color = {0, 0, 127}));
  ...
  annotation(
    Icon(coordinateSystem(extent = {{-200, -200}, {200, 200}}), graphics = {Rectangle(origin = {1, -1}, fillColor = {85, 170, 255}, fillPattern = FillPattern.Solid, lineThickness = 1, extent = {{-181, 161}, {181, -161}}), Text(extent = {{-150, 50}, {150, -50}}, textString = "KK TYP B")}),
    Diagram(coordinateSystem(extent = {{-200, -200}, {200, 200}})));
end Kaeltekreis_TypB_withBoundary;
