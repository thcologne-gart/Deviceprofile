...
model Kaeltekreis_TypB_alt "Modell für Kältekreis Typ B-Beimischschaltung"
  ...
  //-----mixing volume-----
  AixLib.Fluid.MixingVolumes.MixingVolume junc128(redeclare package Medium = Medium, final massDynamics = massDynamics, T_start = T_start, nPorts = 3, final m_flow_nominal = m_flow_nominal, final V = vol, final energyDynamics = energyDynamics) annotation(
 ...
 //----pipe 2-----
  ...
  replaceable AixLib.Systems.HydraulicModules.BaseClasses.BasicPumpInterface PumpInterface(redeclare package Medium = Medium, final allowFlowReversal = allowFlowReversal, final m_flow_nominal = m_flow_nominal, T_start = T_start, final energyDynamics = energyDynamics, final massDynamics = massDynamics) "Needs to be redeclared" annotation(
    Placement(visible = true, transformation(origin = {-36, 0}, extent = {{42, 12}, {58, 28}}, rotation = 0)));
 ...
 AixLib.Fluid.FixedResistances.GenericPipe pipe6(redeclare package Medium = Medium, pipeModel = pipeModel, T_start = T_start, final m_flow_nominal = m_flow_nominal, final allowFlowReversal = allowFlowReversal, length = length, parameterPipe = parameterPipe, parameterIso = parameterIso, final hCon = hCon, final energyDynamics = energyDynamics, final massDynamics = massDynamics) annotation(
  ...
  //-----valve-----
  AixLib.Fluid.Actuators.Valves.ThreeWayTable distributionValve(redeclare package Medium = Medium, CvData = AixLib.Fluid.Types.CvTypes.Kv, Kv = Kv, T_start = T_start, dpFixed_nominal = {10, 10}, energyDynamics = energyDynamics, flowCharacteristics1 = valveCharacteristic.a_ab, flowCharacteristics3 = valveCharacteristic.b_ab, init = Modelica.Blocks.Types.Init.InitialState, m_flow_nominal = m_flow_nominal, order = 1, portFlowDirection_1 = Modelica.Fluid.Types.PortFlowDirection.Leaving, portFlowDirection_2 = Modelica.Fluid.Types.PortFlowDirection.Entering, portFlowDirection_3 = Modelica.Fluid.Types.PortFlowDirection.Leaving, tau = 0.2, y_start = 0) annotation(
    Placement(visible = true, transformation(origin = {-30, -60}, extent = {{-10, 10}, {10, -10}}, rotation = 0)));
  ...
  //-----pipe 8-----
  AixLib.Fluid.FixedResistances.GenericPipe pipe8(redeclare package Medium = Medium, pipeModel = pipeModel, T_start = T_start, final m_flow_nominal = m_flow_nominal, final allowFlowReversal = allowFlowReversal, length = length, parameterPipe = parameterPipe, parameterIso = parameterIso, final hCon = hCon, final energyDynamics = energyDynamics, final massDynamics = massDynamics) annotation(
...
equation
  connect(hydraulicBus.pumpBus, PumpInterface.pumpBus) annotation(
    Line(points = {{0, 100}, {14, 100}, {14, 28}}, color = {255, 204, 51}, thickness = 0.5));
  ...
  connect(pipe4.port_b, senT_b1.port_a) annotation(
 ...
 connect(senT_a2.port_b, pipe5.port_a) annotation(
    Line(points = {{72, -60}, {66, -60}}, color = {0, 127, 255}));
 connect(pipe5.port_b, hydraulic_balancing.port_a) annotation(
    Line(points = {{46, -60}, {40, -60}}, color = {0, 127, 255}));
 connect(hydraulic_balancing.port_b, pipe6.port_a) annotation(
    Line(points = {{20, -60}, {10, -60}}, color = {0, 127, 255}));
 connect(senT_b2.port_a, pipe7.port_b) annotation(
    Line(points = {{-78, -60}, {-70, -60}}));
 connect(pipe8.port_b, junc128.ports[3]) annotation(
    Line(points = {{-40, -10}, {-40, 20}}, color = {0, 127, 255}));
 connect(pipe8.heatPort, prescribedTemperature.port) annotation(
 ...
 connect(pipe8.port_a, distributionValve.port_3) annotation(
 ...
 connect(distributionValve.y_actual, hydraulicBus.valveMea) annotation(
    Line(points = {{-25, -67}, {-25, -90}, {-122, -90}, {-122, 100}, {0, 100}}, color = {0, 0, 127}));
 connect(distributionValve.port_1, pipe7.port_a) annotation(
    Line(points = {{-40, -60}, {-50, -60}}, color = {0, 127, 255}));
 connect(distributionValve.port_2, pipe6.port_b) annotation(
    Line(points = {{-20, -60}, {-10, -60}}, color = {0, 127, 255}));
end Kaeltekreis_TypB_alt;
