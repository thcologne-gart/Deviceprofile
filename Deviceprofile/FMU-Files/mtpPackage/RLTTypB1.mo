...
model RLTTypB1 "Generic air-handling unit with heat recovery system"
  ...
  Modelica.Fluid.Interfaces.FluidPort_a port_a3(redeclare package Medium = Medium2) "Fluid connector a3 (positive design flow direction is from port_a3 to port_b3)" annotation(
    Placement(visible = true, transformation(origin = {40, -100}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {100, -100}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  ...
  AixLib.Fluid.Humidifiers.GenericHumidifier_u humidifierSup(redeclare package Medium = Medium1, final allowFlowReversal = allowFlowReversal1, final m_flow_nominal = m1_flow_nominal, energyDynamics = energyDynamics, T_start = T_start) if useHumidifier "Steam or adiabatic humdifier in supply canal" annotation(
  ...
  AixLib.Fluid.Sensors.Pressure senPreOda annotation(
  ...
  replaceable AixLib.Systems.HydraulicModules.BaseClasses.BasicPumpInterface PumpInterface(redeclare package Medium = Medium2, final allowFlowReversal = false, final m_flow_nominal = m_flow_nominal_kvs, T_start = T_start, final energyDynamics = energyDynamics, final massDynamics = massDynamics) annotation(
    Placement(visible = true, transformation(origin = {14, 30}, extent = {{-42, 12}, {-58, 28}}, rotation = 0)));
  AixLib.Fluid.MixingVolumes.MixingVolume junc1(redeclare package Medium = Medium2, V = vol, nPorts = 3) annotation(
    Placement(visible = true, transformation(origin = {-70, 60}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
  AixLib.Fluid.MixingVolumes.MixingVolume junc2(redeclare package Medium = Medium2, V = vol, nPorts = 3) annotation(
...
equation
  ...
  connect(flapSup.y_actual, genericAHUBus_extended.flapSupMea) annotation(
    Line(points = {{285, -33}, {290, -33}, {290, 44}, {352, 44}, {352, 180}, {0, 180}}, color = {0, 0, 127}));
  ...
  connect(PumpInterface.port_a, senTempKvsIn.port) annotation(
    Line(points = {{-28, 50}, {-8, 50}}, color = {0, 127, 255}));
  connect(PumpInterface.port_b, junc1.ports[1]) annotation(
    Line(points = {{-44, 50}, {-70, 50}}, color = {0, 127, 255}));
  ...
  connect(humidifierSup.powerEva, genericAHUBus_extended.powerHumSupMea) annotation(
    Line(points = {{202, -30}, {208, -30}, {208, 52}, {344, 52}, {344, 180}, {0, 180}}, color = {0, 0, 127}));
  ...
  connect(boundary_b3.ports[1], port_b3.port_b3) annotation(
  ...
  connect(boundary_a1.ports[1], port_a1) annotation(
    Line(points = {{-362, -40}, {-300, -40}}, color = {0, 127, 255}));
  connect(boundary_b1.ports[1], port_b1) annotation(
    Line(points = {{362, -40}, {300, -40}}, color = {0, 127, 255}));
  connect(boundary_a2.ports[1], port_a2) annotation(
    Line(points = {{362, 140}, {300, 140}}, color = {0, 127, 255}));
  annotation(
    Icon(coordinateSystem(preserveAspectRatio = false, extent = {{-300, -100}, {300, 200}}), graphics = {Rectangle(origin = {0, 10}, fillColor = {170, 255, 127}, fillPattern = FillPattern.Solid, lineThickness = 1, extent = {{-200, 90}, {200, -90}}), Text(origin = {0, 10}, extent = {{-200, 90}, {200, -90}}, textString = "RLT TYP B1", fontSize = 72)}),
